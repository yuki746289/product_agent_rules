...
プロジェクト直下の `.document` フォルダに作成した `仕様_差分_v{n}.{m}.md` および `仕様_全体_v{n}.{m}.md` が、`.rules/dev/quick_fix/rules_quick_fix_4_spec.md` の制約を満たし、前バージョンからの仕様継承を漏れなく反映しているかを確認して下さい。
...
不備がある場合は、`rules_quick_fix_4_spec.md` に戻って仕様書を修正して下さい。
